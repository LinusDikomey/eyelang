use ir2::{
    ...
    BlockTarget, Environment, Function, ModuleId, ModuleOf, Parameter, Ref,
};
...
fn main() {
    ...
    let cond = Ref::TRUE;
    // TODO: make builder generated by macro work with non-static lifetimes
    builder.append(
        (
            ir2::FunctionId {
                module: cf.id(),
    ...
    let my_res = builder.append(
        (my_function, (params.nth(0), params.nth(1), result)),
        int_ty,
    );
    ...
    let my_add = env.add_function(main, builder.finish(int_ty));
    ...
    for block in ir.block_ids() {
        for (_r, inst) in ir.get_block(block) {
            if let Some(inst) = inst.as_module(modules.arith) {
                match inst.op() {
                    Arith::Int => {
                        println!("int with {:?}", ir.args(&inst).collect::<Vec<_>>());
                    }
                    Arith::Add => {
                        println!("add with {:?}", ir.args(&inst).collect::<Vec<_>>());
                    }
                    _ => {}
                }
                ...
                match inst.op() {
                    ...
                    Cf::Branch => {
                        println!("branch found with {:?}", ir.args(&inst).collect::<Vec<_>>())
                    }
                    _ => {}
                }
