...
use crate::{
    Compiler, Def,
    compiler::{
        CheckedFunction, Generics, LocalScopeParent, Resolvable, ResolvedEnumDef,
        ResolvedStructDef, ResolvedTypeContent, ResolvedTypeDef, Signature, VarId,
    },
    error::{CompileError, Error},
    hir::{CastId, HIRBuilder, Hir, LValue, Node, NodeId},
    parser::ast::{self, Ast, ExprId, ScopeId, TraitId},
    types::{LocalTypeId, LocalTypeIds, TypeInfo, TypeInfoOrIdx, TypeTable},
};

use self::exhaust::Exhaustion;

pub(crate) fn ty(compiler: &mut Compiler, ty: TypeId) -> ResolvedTypeDef {
    ...
    // TODO: might need to check impls in a separate step after resolved type is available
