...
fn main() {
    ...
    builder.append(mem.Store(variable, counter));
    let zero = builder.append(arith.Int(0, int_ty));
    builder.append(cf.Goto(BlockTarget::new(loop_body)));
    ...
    builder.append(mem.Store(sum, new_sum));
    let new_value = builder.append(arith.Sub(loaded, one, int_ty));
    builder.append(mem.Store(variable, new_value));
    let is_zero = builder.append(arith.Eq(new_value, zero, i1_ty));
    builder.append(cf.Branch(is_zero, BlockTarget::new(end), BlockTarget::new(loop_body)));

    builder.begin_block(end, []);
    let loaded_sum = builder.append(mem.Load(sum, int_ty));
    builder.append(cf.Ret(loaded_sum));
