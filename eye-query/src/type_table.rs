use crate::{id, Primitive, TypeDefId, ResolvedType};
...
impl TypeTable {
    ...
    pub fn from_resolved(&mut self, ty: &ResolvedType) -> LocalTypeId {
    ...
    pub fn info_from_resolved(&mut self, ty: &ResolvedType) -> TypeInfo {
        match ty {
            &ResolvedType::Primitive(p) => TypeInfo::Primitive(p),
            ResolvedType::Pointer(inner) => TypeInfo::Pointer(self.from_resolved(inner)),
            ResolvedType::Array(b) => {
                let (elem_ty, count) = &**b;
                let element = self.from_resolved(elem_ty);
                TypeInfo::Array { element, count: Some(*count) }
            }
            ResolvedType::Def { id, generics } => {
