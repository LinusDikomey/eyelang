...
Queue :: struct[T] {
    ...
    new :: fn -> Queue[T]: Queue(buf: root.null(), len: 0, cap: 0, head: 0)
    ...
    realloc_to_cap :: fn(this *Queue[T], new_cap u64) {
        ...
        if this.len != 0 {
            if this.head + this.len <= this.cap {
                memcpy(new_buf as *u8, ptr_add(this.buf, this.head) as *u8, this.len * T.stride)
            } else {
                first_len := this.cap - this.head
                second_len := this.len - first_len
                memcpy(new_buf as *u8, ptr_add(this.buf, this.head) as *u8, first_len * T.stride)
                memcpy(ptr_add(new_buf, first_len) as *u8, this.buf as *u8, second_len * T.stride)
            }
        }
        this.head = 0
        this.buf = new_buf
